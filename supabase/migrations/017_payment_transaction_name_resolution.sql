...
-- Step 9.5 (Payment): post_payment_transaction previously required raw
-- contact_id / bank_account_id / invoice_id UUIDs, which chat extraction
-- never has (it only has names/numbers). This mirrors the find-or-create
-- pattern post_record_transaction already uses for sale/purchase contacts
-- (008_phase2_accounting_spine.sql:214-218):
--   - contact: resolve by name, create if missing (same as sale/purchase)
--   - bank account: resolve by name, but NEVER auto-create — it's a small
--     controlled org-level list an accountant sets up, not something a chat
--     model should invent
--   - invoice: resolve by invoice_number
-- Raw UUIDs are still accepted for backward compatibility with any existing
-- caller that already resolves IDs itself.
...
DECLARE
  ...
  v_contact UUID := NULLIF(v_payment->>'contact_id','')::UUID;
  v_party TEXT := COALESCE(NULLIF(v_payment->>'party',''), NULLIF(v_payment->>'partyName',''));
  v_bank_account UUID := NULLIF(v_payment->>'bank_account_id','')::UUID;
  v_bank_account_name TEXT := NULLIF(v_payment->>'bank_account_name','');
  v_bank UUID;
  ...
  v_invoice UUID; v_invoice_number TEXT; v_alloc NUMERIC; v_available NUMERIC; v_invoice_balance NUMERIC; v_allocated NUMERIC := 0;
...
BEGIN
  ...
  -- Contact: resolve by id if given, else find-or-create by name (same rule
  -- post_record_transaction uses for sale/purchase counterparties).
  IF v_contact IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM contacts WHERE org_id=v_org AND id=v_contact) THEN
      RAISE EXCEPTION 'Contact is not in this organization';
    END IF;
  ELSE
    IF v_party IS NULL THEN RAISE EXCEPTION 'Payment requires a contact_id or party name'; END IF;
    SELECT id INTO v_contact FROM contacts WHERE org_id=v_org AND name=v_party LIMIT 1;
    IF v_contact IS NULL THEN
      INSERT INTO contacts(org_id,name,contact_type)
      VALUES (v_org, v_party, CASE WHEN v_type='received' THEN 'customer' ELSE 'vendor' END)
      RETURNING id INTO v_contact;
    END IF;
  END IF;

  -- Bank account: resolve by id if given, else by name. Never auto-created —
  -- it's a controlled org-level list, not something chat extraction should guess.
  ...
  ELSIF v_bank_account_name IS NOT NULL THEN
    SELECT ledger_account_id INTO v_bank FROM bank_accounts
      WHERE org_id=v_org AND is_active AND (account_name ILIKE v_bank_account_name OR bank_name ILIKE v_bank_account_name)
      ORDER BY created_at ASC LIMIT 1;
    IF v_bank IS NULL THEN RAISE EXCEPTION 'No matching bank account named "%" was found in this organization', v_bank_account_name; END IF;
  ELSE
    RAISE EXCEPTION 'Payment requires a bank_account_id or bank_account_name';
  END IF;
  ...
  FOR v_line IN SELECT value FROM jsonb_array_elements(v_allocs) LOOP
    v_invoice := NULLIF(v_line->>'invoice_id','')::UUID;
    v_invoice_number := NULLIF(v_line->>'invoice_number','');
    IF v_invoice IS NULL THEN
      IF v_invoice_number IS NULL THEN RAISE EXCEPTION 'Each allocation requires an invoice_id or invoice_number'; END IF;
      SELECT id INTO v_invoice FROM invoices WHERE org_id=v_org AND invoice_number=v_invoice_number AND status <> 'void' LIMIT 1;
      IF v_invoice IS NULL THEN RAISE EXCEPTION 'No invoice numbered "%" was found in this organization', v_invoice_number; END IF;
    END IF;
    v_alloc := (v_line->>'amount')::NUMERIC;
  ...
  RETURN jsonb_build_object('success',true,'duplicate',false,'paymentId',v_payment_id,'journalEntryId',v_journal_id,'contactId',v_contact,'bankAccountLedgerId',v_bank);
END; $fn$;
