-- Atomic record posting. The API prepares a validated request; this function owns
-- every write and PostgreSQL commits or rolls back the entire operation.
...
BEGIN
  ...
  IF v_type IN ('sale','purchase') THEN
    ...
    INSERT INTO invoices(org_id,contact_id,invoice_type,invoice_number,issue_date,due_date,emirate,currency,exchange_rate,subtotal_amount,vat_amount,discount_amount,total_amount,is_reverse_charge,status)
    VALUES (v_org,v_contact_id,CASE WHEN v_type='sale' THEN 'sales_invoice'::invoice_type ELSE 'purchase_invoice'::invoice_type END,v_number,v_issue_date,COALESCE(NULLIF(v_record->>'dueDate','')::DATE,v_issue_date),COALESCE(NULLIF(v_record->>'emirate',''),'Dubai')::emirate_enum,COALESCE(NULLIF(v_record->>'currency',''),'AED'),COALESCE(NULLIF(v_record->>'exchangeRate','')::NUMERIC,1),COALESCE((v_totals->>'subtotal')::NUMERIC,0),COALESCE((v_totals->>'vat')::NUMERIC,0),COALESCE((v_totals->>'discount')::NUMERIC,0),COALESCE((v_totals->>'total')::NUMERIC,0),COALESCE((v_record->>'reverseCharge')::BOOLEAN,false),'approved') RETURNING id INTO v_invoice_id;
  ...
  -- Journal lines are prepared by the route but resolved and inserted here.
  IF jsonb_array_length(COALESCE(p_request->'journal_lines','[]'::jsonb)) > 0 THEN
    ...
    END LOOP;
    IF ABS(v_total_debit-v_total_credit) > .005 OR v_total_debit=0 THEN RAISE EXCEPTION 'Unbalanced journal'; END IF;
